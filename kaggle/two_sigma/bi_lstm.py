...
train = pd.read_hdf('../kaggle_data/train.h5') # for local data
...
fields_needed =["technical_20","timestamp", 'id']
# fields_needed = ['fundamental_18', 'fundamental_2', 'fundamental_11', 'fundamental_16', 'fundamental_3', 'fundamental_9', 'technical_16', 'technical_19', 'fundamental_8', 'technical_25']
...
def batch_gen_2d(batch_size):
    while True:
        ...
        x_batch = map(lambda x: np.reshape(np.array(x), (10, x_train.shape[1])),x_batch) # 10 is nu of recurrent layers
...
batch_size = 500
validation_data = (train[fields_needed][-1000:], train.y[-1000:])
input_shape = (10,x_train.shape[1])
print("initializing model...")
model = compile_model_2d(input_shape)
print("fitting model...")
fit = model.fit_generator(generator=batch_gen_2d(500), nb_epoch=50, validation_data = batch_gen_2d(500), nb_val_samples=500, samples_per_epoch=train_size)
...
#     observation.target.y = model.predict(x_test)
#     observation.target.y[observation.target.y>max_y] = max_y
#     observation.target.y[observation.target.y<min_y] = min_y
