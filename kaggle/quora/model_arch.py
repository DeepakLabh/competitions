...
def siamese(max_sent_len, wordvec_dim,gru_output_dim, output_dim):
    gru_sent_fwd = GRU(gru_output_dim, return_sequences=False, go_backwards = False, activation='tanh', inner_activation='hard_sigmoid', input_shape = (max_sent_len,wordvec_dim))
    gru_sent_bwd = GRU(gru_output_dim, return_sequences=False, go_backwards = True, activation='tanh', inner_activation='hard_sigmoid', input_shape = (max_sent_len,wordvec_dim))
    #input_layers = []
	...
	gru_f = Activation('relu')(gru_f)
        gru_b = Activation('relu')(gru_b)
	gru_merge = merge([gru_f, gru_b], mode = 'concat', concat_axis = -1)
        gru_merge = Dropout(0.25)(gru_merge)
        gru_merge = Dense(32)(gru_merge)
        gru_merge = Dense(2)(gru_merge)
